...
arma::mat update_resp_fun(arma::mat props,arma::vec beta_nb,double alpha_nb, arma::vec beta_pl, double c_pl,arma::mat x_nb_ext, arma::mat x_pl_ext, arma::vec y){
  ...
  for (int i=0; i<n; i++){
    ...
    if(y(i)==0){
      ...
      double d_nb = exp(ell_nb_i_fun(beta_nb,alpha_nb,trans(x_nb_ext.submat(i,0,i,n_nb-1)),y(i)));
      double d_pl = ell_pl_i_fun(beta_pl,c_pl,trans(x_pl_ext.submat(i,0,i,n_pl-1)),y(i));
...
//[[Rcpp::export]]
arma::vec update_theta_nb_fun(arma::mat resp, arma::vec beta_nb, double alpha_nb, arma::mat x_nb_ext, arma::mat x_pl_ext, arma::vec y, int no_m_bfgs_steps, double max_upd_par){
  int n = x_nb_ext.n_rows;
  int n_nb = x_nb_ext.n_cols;
  int n_pl = x_pl_ext.n_cols;
